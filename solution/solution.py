...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            "rubric criterion.\n"
            "Use a score from 0.0 to 1.0, where 0.0 means the criterion is not "
            "met and 1.0 means it is fully met. Judge the content, not response "
            "length or presentation order.\n\n"
            f"Question:\n{question}\n\n"
            f"Answer:\n{answer}\n\n"
            f"Rubric:\n{rubric_json}\n\n"
            "Return only one JSON object mapping each rubric criterion to its "
            "numeric score."
        )
        ...
        try:
            ...
            # LLMs sometimes wrap an otherwise valid JSON object in a fenced
            # block. Accept that common format while retaining the safe
            # per-criterion fallback below for malformed output.
        ...
        for criterion in rubric:
            ...
            if not math.isfinite(numeric_value):
                ...
                scores[criterion] = numeric_value
            elif 1.0 < numeric_value <= 5.0:
                # Support the lecture's 1–5 judge scale while exposing the
                # normalized 0–1 interface promised by this method.
                scores[criterion] = numeric_value / 5.0
            else:
                scores[criterion] = 0.5
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        return {
            ...
            "leniency_bias": (
                batch_average is not None and batch_average > 0.8
            ),
            "severity_bias": (
                batch_average is not None and batch_average < 0.3
            ),
        }
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        new_averages = {
            metric: average_metric(new_results, metric) for metric in metrics
        }
        baseline_averages = {
            metric: average_metric(baseline_results, metric) for metric in metrics
        }
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        fixes_by_type = {
            "hallucination": (
                "Route scope and safety intents to authoritative context, then "
                "reject claims that are not grounded in retrieved evidence"
            ),
            "irrelevant": (
                "Add structured safe-response examples that explain the refusal "
                "and the applicable OrbitTech policy"
            ),
            "incomplete": (
                "Require an answer checklist covering every requested condition "
                "and exception"
            ),
            "off_topic": (
                "Require an answer checklist covering every question sub-part "
                "before returning the response"
            ),
            "refusal": (
                "Calibrate guardrails with allowed support examples to reduce "
        ...
        for index, failure in enumerate(failures, start=1):
            ...
            suggestion = fixes_by_type.get(
                failure_type.casefold(),
                suggestions[index - 1]
                if index <= len(suggestions)
                else (
                    "Inspect the answer and retrieval trace, then add a "
                    "regression case"
                ),
            )
            rows.append(
    ...
    def generate_improvement_suggestions(
        ...
        suggestion_map = {
            "hallucination": (
                "Add claim-level grounding checks and require evidence-backed "
                "answers before returning a response"
            ),
            "irrelevant": (
                "Improve intent routing and add few-shot examples that answer "
                "the user's question directly"
            ),
            "incomplete": (
                "Expand or rerank retrieved context and add answer checklists "
                "for required conditions and exceptions"
            ),
            "off_topic": (
                "Tighten the domain prompt and route unsupported intents to a "
                "clear scope-aware response"
            ),
            "refusal": (
                "Calibrate safety guardrails with allowed customer-support "
                "examples to reduce unnecessary refusals"
            ),
            "unknown": (
                "Inspect generation and retrieval traces, assign a failure type, "
                "and add the case to the regression suite"
            ),
        }
        ...
        suggestions = [
            suggestion_map.get(failure_type, suggestion_map["unknown"])
            for failure_type in ordered_types
        ]

        general_suggestions = [
            "Review the lowest-scoring retrieval traces and tune query, chunking, and top-k settings",
            "Add representative failures to the golden dataset and enforce metric regression gates in CI",
            "Calibrate heuristic and LLM-judge scores against human labels before changing deployment thresholds",
        ]
